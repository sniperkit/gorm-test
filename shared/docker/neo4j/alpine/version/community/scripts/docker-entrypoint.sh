...
export NEO4J_GROUP=${NEO4J_GROUP:-"neo4j"}
export NEO4J_HOME=${NEO4J_HOME:-"/var/lib/neo4j"}

# directories
export NEO4J_CERTS_DIR=${NEO4J_CERTS_DIR:-"/opt/neo4j/ssl"}
export NEO4J_CONF_DIR=${NEO4J_CONF_DIR:-"/opt/neo4j/conf"}
export NEO4J_DATA_DIR=${NEO4J_DATA_DIR:-"/opt/neo4j/data"}
export NEO4J_LOGS_DIR=${NEO4J_LOGS_DIR:-"/opt/neo4j/logs"}
export NEO4J_IMPORT_DIR=${NEO4J_IMPORT_DIR:-"/opt/neo4j/import"}
export NEO4J_PLUGINS_DIR=${NEO4J_PLUGINS_DIR:-"/opt/neo4j/plugins"}
export NEO4J_METRICS_DIR=${NEO4J_METRICS_DIR:-"/opt/neo4j/metrics"}

# commands
export NEO4J_CMD_DEFAULT=${NEO4J_CMD_DEFAULT:-"neo4j"}
export NEO4J_CMD_CONSOLE=${NEO4J_CMD_CONSOLE:-"console"}
...
if [ "$(id -u)" = "0" ]; then
  userid="${NEO4J_USER}"
  groupid="${NEO4J_GROUP}"
  exec_cmd="exec su-exec ${NEO4J_CMD_DEFAULT}"
else
  userid="$(id -u)"
  groupid="$(id -g)"
  exec_cmd="exec"
...
do
  if [[ "$(id -u)" = "0" ]] && [[ "$(stat -c %U "${dir}")" = "${NEO4J_USER}" ]]; then
...
if [[ "${cmd}" != *"neo4j"* ]]; then
  if [ "${cmd}" == "dump-config" ]; then
    if [ -d /conf ]; then
      ${exec_cmd} cp --recursive conf/* ${NEO4J_CONF_DIR}
      exit 0
    else
...
if [ -d "${NEO4J_CONF_DIR}" ]; then
    find "${NEO4J_CONF_DIR}" -type f -exec cp {} conf \;
fi

if [ -d "${NEO4J_CERTS_DIR}" ]; then
    NEO4J_dbms_directories_certificates="${NEO4J_CERTS_DIR}"
fi

if [ -d "${NEO4J_PLUGINS_DIR}" ]; then
    NEO4J_dbms_directories_plugins="${NEO4J_PLUGINS_DIR}"
fi

if [ -d "${NEO4J_LOGS_DIR}" ]; then
    NEO4J_dbms_directories_logs="${NEO4J_LOGS_DIR}"
fi

if [ -d "${NEO4J_IMPORT_DIR}" ]; then
    NEO4J_dbms_directories_import="${NEO4J_IMPORT_DIR}"
fi

if [ -d "${NEO4J_METRICS_DIR}" ]; then
    NEO4J_dbms_directories_metrics="${NEO4J_METRICS_DIR}"
fi
...
if [[ "$(id -u)" = "0" ]]; then
  ...
  chown -R "${userid}":"${groupid}" ${NEO4J_DATA_DIR}
fi
...
if [ "${cmd}" == "${NEO4J_CMD_DEFAULT}" ]; then
  ${exec_cmd} ${NEO4J_CMD_DEFAULT} ${NEO4J_CMD_CONSOLE}
else
  ${exec_cmd} "$@"
fi
